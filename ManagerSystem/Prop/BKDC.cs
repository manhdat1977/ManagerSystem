...
namespace ManagerSystem.Prop
{
    public class BKDC
    {
        ...
        public string DMuc_HoNgheo { get; set; }
        ...
        public string TLSHoatDC { get; set; }
        public string TLCQuanDC { get; set; }
        public string TLSXuatDC { get; set; }
        public string TLKDoanhDC { get; set; }
        public string DMucDC { get; set; }
        ...
        public string DMuc_HoNgheoDC { get; set; }
        ...
        public int CheckMaDon(BKDC objBKDC)
        {
            conn = condb.connectSQL_Bang_Ke_Dieu_Chinh();
            conn.Open();
            cmd = new SqlCommand("select count(*) from BKDC_Table1 where MaSNDon = '" + objBKDC.MaSNDon + "'", conn);
            Int32 i = (Int32)cmd.ExecuteScalar();
            conn.Close();
            return i;
        }

        public int CheckDanhBaIsTrue(BKDC objBKDC)
        {
            Int32 i = 0;
            conn = condb.connectSQL_Bang_Ke_Dieu_Chinh();
            if (conn.State != ConnectionState.Open)
            ...
            cmd = new SqlCommand("select count(*) from BKDC_Table1 where InBD = 1 and dbo = '" + objBKDC.Dbo + "'", conn);
            i = (Int32)cmd.ExecuteScalar();
            conn.Close();
            return i;
        }

        public int UpdateBKDC(BKDC objBKDC)
        {
            int i = 0;
            conn = condb.connectSQL_Bang_Ke_Dieu_Chinh();
            if (conn.State != ConnectionState.Open)
            {
                conn.Open();
                cmd = new SqlCommand("proc_update_BKDC", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@id", objBKDC.id);
                cmd.Parameters.AddWithValue("@KHangDC", objBKDC.KHangDC);
                cmd.Parameters.AddWithValue("@DChiDC", objBKDC.DChiDC);
                cmd.Parameters.AddWithValue("@DChi_TruSoDC", objBKDC.DChi_TruSoDC);
                cmd.Parameters.AddWithValue("@MSThueDC", objBKDC.MSThueDC);
